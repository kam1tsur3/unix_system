...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		if(select(sd+1, &rdfds, NULL, NULL, &timeout) == 0){
			if(sec_pass > 0){
				for(tmp_client = used_head.fp; tmp_client != &used_head; tmp_client = tmp_client->fp){
					tmp_client->ttlcounter -= sec_pass;
					if(tmp_client->ttlcounter > 0)
						continue;
					printf("------------------------------\n");
					switch(tmp_client->status){
						case 1:
							s_buf[0] = '\x02';
							s_buf[1] = '\x00';
							*((short *)&s_buf[2]) = tmp_client->ttl;
							...
							*((in_addr_t *)&s_buf[8]) = tmp_client->netmask.s_addr;
							...
							tmp_client->status = 2;
							tmp_client->ttlcounter = tmp_client->ttl;
